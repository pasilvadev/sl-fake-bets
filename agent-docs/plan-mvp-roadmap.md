...
- **Deleting a resolved bet can be refused.** Unwinding a resolved bet subtracts a payout the winner may already have staked elsewhere, and DOM-014 forbids a negative balance. The spec says nothing about this case, so `delete_bet` pre-checks and raises a readable sentence instead of failing on the `coin_balance >= 0` CHECK. It is the one place this phase had to answer a question the spec does not — flagging it rather than burying it.
