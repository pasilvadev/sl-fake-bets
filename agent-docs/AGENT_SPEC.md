...
| `plan-hosted-early-access.md` | The ARC-012 hosted transition and the step past it — a friends-only early access on Supabase Cloud free + Vercel Hobby with the owner developing Docker-free. Web-verified vendor facts (2026-09-07), twelve decisions (Google + unverified email/password for the alpha with a required typed display name; the email code, password reset and their SMTP vendor registered for full release; two Supabase projects; a guarded prod-push script; a Vercel keep-alive cron), a one-sitting owner checklist with direct links (Phase 0), and five agent phases. Status: IN EXECUTION (2026-09-07) — Phases 1 and 2 complete; Phase 3's infrastructure (schema on prod, production deploy live at `https://sl-fake-bets.vercel.app`, Google + password auth verified, keep-alive cron registered) is complete, with only human-and-device tasks (a stranger's phone, the owner's own production sign-in) left to the owner; Phase 4 (docs and the Docker-free loop) in progress |
| `plan-invite-links.md` | Invite links: expiry choice & revocation — closes open decision #6 and amends UX-005/DOM-005 (D12), on an owner order given in chat 2026-09-07. `invite_codes` gains a nullable `expires_at` (`NULL` = permanent, still the default a team is born with); a member may also mint an opt-in 24-hour link, capped at one live permanent and ten live 24-hour links per team; revocation is per-link, by the leader, any moderator, or whoever made that link. Both writes become SECURITY DEFINER RPCs (`create_invite_code`/`revoke_invite_code`) — the last direct client write on a domain table (`invite_codes`) is withdrawn, completing the Phase 5 tightening. `Team.inviteCode` becomes `Team.invites: TeamInvite[]`; the invite modal becomes a small link manager (live links with kind and countdown, per-link revoke, a permanent/24-hour composer). `vision.md`'s "try as hard as possible not to expire invite links" pillar is unchanged — nothing expires a link nobody asked to expire |
...
- **UX-002 [mvp]** Every customizable onboarding input (display name, avatar, name color, etc.) ships pre-filled with an auto-generated placeholder so lazy users skip through with a valid profile; users who care can customize during onboarding or later in profile settings. *(Carve-out, owner ruling 2026-09-07 — D8 of `plan-hosted-early-access.md`: the hosted early access's password create-account path requires a typed display name, since a password sign-up has nothing to pre-fill one from; Google sign-ups still arrive pre-filled from the provider, and every other onboarding input on every other path is unaffected.)*
...
- **UX-005 [mvp]** ~~Team invite links must not expire (non-expiring or very long-lived).~~ Permanent by default and still what a team is born with — no automatic expiry is ever applied to a link nobody chose to expire; a member may additionally opt one link into a 24-hour expiry. ~~Open: manual revoke/regenerate by leader/moderator unspecified.~~ *(Amended, owner order 2026-09-07 — D12 of `plan-invite-links.md`: resolves open decision #6 (§6 item 6); manual revocation is per-link, by the leader, any moderator, or the link's creator (that plan's D5). `vision.md`'s "try as hard as possible not to expire invite links" pillar is unaffected — nothing here expires a link nobody asked to expire.)*
...
- **DOM-005 [mvp]** ~~Invites never expire by time (product pillar).~~ Permanent by default, never expired automatically; an opt-in 24-hour link is the one exception, chosen per link by whoever makes it, never imposed on the default. ~~Open: manual revocation unspecified.~~ *(Amended, owner order 2026-09-07 — D12 of `plan-invite-links.md`: resolves open decision #6 (§6 item 6); revocation is a per-link action by the leader, any moderator, or the link's creator (that plan's D5), enforced in the `revoke_invite_code` RPC.)*
...
- **DOM-022 [mvp]** Daily login reward: first login of each calendar day grants coins, reference ~~5~~ **10** — tunable config. Open: vision frames it as helping broke users — unconditional daily grant vs only-when-depleted is ambiguous; default assumption: unconditional. *(Amended, owner order 2026-09-09: the reference value is now **10** coins — `CONFIG.DAILY_REWARD_COINS` and its SQL twin `app.daily_reward_coins()` (`supabase/migrations/20260909100100_login_rewards_retune.sql`) moved together. Assumption A-2 (unconditional) is unchanged.)*
...
- **DOM-036 [post-mvp]** Weekly login reward: 100 coins, unconditional, once per (user, team, ISO week in UTC — Monday 00:00 UTC), granted lazily on team load exactly as DOM-022's daily reward is, as its own ledger kind `weekly-reward` (DOM-025) with idempotence owned by a partial unique index. Owner order 2026-09-09, after the local-MVP checkpoint — hence the tag, per §3's rule for owner-ordered post-checkpoint features (it ships with no feature flag: a grant amount is a tunable, not a surface, and a flag that could withhold coins the ledger promises would be a leak, not a kill switch). Tunable: `CONFIG.WEEKLY_REWARD_COINS` with SQL twin `app.weekly_reward_coins()` in `20260909100100_login_rewards_retune.sql`. Not a donation (DOM-023 stays excluded) and not a notification (ARC-014 untouched): the wallet shows a passive "Weekly login: +100 this week ✓" readout, never a claim button or an alert.
...
- **ARC-006 [mvp]** Only email login + Google OAuth. Open: "email" flavor unspecified — magic link, OTP code, or email+password; pick lowest-friction available in chosen stack. **Resolved 2026-09-07** (§6 item 7; D12 of `plan-hosted-early-access.md`): password login, unverified, for the hosted early access — the built-in mailer cannot reach anyone outside the project's own Supabase team without a third-party SMTP vendor, and no vendor is added for the alpha. The OTP code and "forgot password" return at full release behind that vendor (that plan's §8); this does not reopen the flavor question, it defers it.
...
- **ARC-012 [mvp]** Phase 3: hosted backend, still dev/staging, not production. Open: production phase undefined. **Entered 2026-09-07** (owner's ARC-013 order; `plan-hosted-early-access.md`): the hosted early access — friends signing themselves up on Supabase Cloud free + Vercel Hobby, still free-tier and still "early access" per ARC-001 — is the step past this requirement's "still dev/staging" wording; see §7. A full, non-early-access production phase remains open and undefined.
...
6. ~~Invite link manual revocation (UX-005/DOM-005).~~ **Resolved 2026-09-07** — `plan-invite-links.md` D5/D8.
7. Email auth flavor: magic link / OTP / password (ARC-006).
8. Default team choice for multi-team users on app open (UX-011).

## 7. Current dev phase

**Phase 3 (ARC-012): hosted backend, early access.** Entered 2026-09-07 on the owner's explicit ARC-013 order for the 2→3 move (`plan-hosted-early-access.md`, itself the ARC-013 order for that move and the step past this requirement's "still dev/staging" wording). Two Supabase Cloud free projects in one org — `sl-fake-bets` (production, friends) and `sl-fake-bets-dev` (the owner's Docker-free daily driver) — plus one Vercel Hobby project, live at `https://sl-fake-bets.vercel.app`. Still free-tier and still "early access" per ARC-001; see `supabase/README.md` §Hosted for the day-to-day mechanics.

~~Phase 2 (ARC-011): fully local backend.~~ **Superseded 2026-09-07.** Phase 2 was entered 2026-09-05 on the owner's explicit ARC-013 order for the 1→2 move; Supabase ran 100% on the developer machine via CLI + Docker, with full schema, RLS, storage and local auth providers in place (`supabase/`), seeded from `mock-data.ts`. That local stack still exists and still works — `supabase/README.md`'s appendix is how to reach it — but it is no longer the owner's daily driver, and the CLI now stays linked to the hosted dev project rather than to a local one.

~~The frontend has NOT been migrated onto it yet — `apps/web` still runs on in-memory state, and there are no RPCs. That migration is roadmap Phases 4–9 (`plan-mvp-roadmap.md`).~~ **Superseded 2026-09-07:** that migration is done. Roadmap Phases 1–9 are all complete — `apps/web` reads and writes Postgres through RPCs, realtime, RLS and real auth, and the local-MVP checkpoint was reached at Phase 9. Work since then is `plan-mvp-roadmap.md` §8's owner-ordered extra phases (team chat, 1v1 duels, a real toast system, pt-BR) and this hosted transition — neither reopens the closed roadmap.

~~Phase 3 (ARC-012, hosted backend) requires its own separate, explicit, in-the-moment owner order. Finishing Phase 2 does not authorize it.~~ **That order was given 2026-09-07**, in chat, immediately before the first production schema push: "Execute phase 3 of the plan. Trust the plan." (`plan-hosted-early-access.md`'s own execution record). ARC-013's rule is unchanged by having been satisfied once — whatever comes after this early access (a full, verified-email, non-"early access" production) still needs its own separate, explicit, in-the-moment owner order, and finishing Phase 3 does not authorize it.
