...
| `plan-hosted-early-access.md` | The ARC-012 hosted transition and the step past it — a friends-only early access on Supabase Cloud free + Vercel Hobby with the owner developing Docker-free. Web-verified vendor facts (2026-09-07), twelve decisions (Google + unverified email/password for the alpha with a required typed display name; the email code, password reset and their SMTP vendor registered for full release; two Supabase projects; a guarded prod-push script; a Vercel keep-alive cron), a one-sitting owner checklist with direct links (Phase 0), and five agent phases. Status: PLAN, awaiting the owner's Phase 0 |
