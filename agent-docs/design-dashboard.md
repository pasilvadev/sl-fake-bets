...
Right, fixed order:
1. **Balance pill** — coin glyph + mono balance (current team, DOM-013); jade `+10` micro-tag docked for the daily grant (DOM-022/A-2 — automatic, never a claim button); click opens Wallet popover-equivalent (transactions modal).
...
1. **Wallet** — big mono balance numeral (N8) + coin glyph; "Daily login: +10 today ✓" line (auto-grant); "Weekly login: +100 this week ✓" line (auto-grant, DOM-036, owner order 2026-09-09); "Your P/L" slash-glyph line (DOM-026); "View transaction history" → modal (DOM-025 dense rows: date · description · delta · balance-after); disabled **Donate Coins** button, tooltip "Coming soon" (DOM-023 future-stub; the long-term home is also a per-name user popover — post-MVP).
