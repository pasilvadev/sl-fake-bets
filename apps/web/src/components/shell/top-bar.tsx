 ...
 * team switcher; right = balance pill, Invite, Create Bet, notifications
 * (future-stub), profile menu — fixed order.
 */
export function TopBar() {
  const { balance, canCreateBet, canInvite } = useTeam();
  const { open } = useModal();
    ...
    <header className="sticky top-0 z-40 flex h-14 shrink-0 items-center gap-3 border-b border-border bg-surface-1 px-4">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <button
          ...
          className={cn(
            "hidden h-8 items-center rounded-sm bg-jade px-3 text-xs font-semibold uppercase text-black transition-[filter] hover:brightness-110 active:brightness-95 sm:inline-flex",
